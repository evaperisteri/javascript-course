...
function renderNotes(){
  ...
  container.innerHTML = notes.map(note => `<div id="${'noteTemplate' + note.key}" class="flex justify-between items-center px-[10px] py-[2px] border-b border-black">
      <div id="${'noteInfo' + note.key}" class="flex items-center"> 
        <input type="checkbox" id="${'noteCheck' + note.key}" onclick="strikeTrough(${note.key})" class = "w-[25px] h-[25px] mr-[5px]" ${note.softDeleted ? 'checked' : ''}>
        <label id="${'noteTxt' + note.key}" for="${'noteCheck' + note.key}" class="w-[200px] max-h-[150px] leading-[1.2rem] break-words whitespace-normal overflow-hidden text-base ${note.softDeleted ? 'line-through text-grey-500' : ''}">${note.note}</label>
      </div>
 ...
 * Model functions
 */
function strikeTrough(key) {
  //Πρακτικα η εναλλαγη ενός toggle 
